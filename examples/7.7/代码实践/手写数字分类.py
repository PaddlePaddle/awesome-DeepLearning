...
#加载测试数据集
test_loader = paddle.io.DataLoader(eval_dataset, batch_size=1000, shuffle=True)
        ...
        for batch_id, (data, target) in enumerate(train_loader):
            ...
            if batch_id % 200 == 0:
                print("epoch: {}, batch: {}, loss is: {}".format(
                    epoch + 1, batch_id, avg_loss.numpy()))
...
with paddle.fluid.dygraph.guard(place):
    ...
    for data, target in test_loader:
        ...
        correct += (np.argmax(
            output, axis=1) == target.numpy().reshape(-1)).sum().item()
