...
train = pd.read_csv(
    "C:/Users/ASUS/Desktop/12/data/diabetes_train.txt",
    header=None,
    index_col=False)
test = pd.read_csv(
    "C:/Users/ASUS/Desktop/12/data/diabetes_test.txt",
    header=None,
    index_col=False)
#数据转换
...
with tf.name_scope("Deep"):
    ...
    print("lay%s, input_size: %s, output_size: %s, active_fuc: %s" %
          (1, n_features * fv, dnn_layer[0], dnn_active_fuc[0]))
    with tf.variable_scope("deep_layer1", reuse=tf.AUTO_REUSE):
        ...
        deep_inputs = tf.reshape(
            embeddings, shape=[-1, input_size])  # None * (F*K)
        print("%s: %s" % ("lay1, deep_inputs", deep_inputs))

        # 全连接计算    
        deep_outputs = udf_full_connect(deep_inputs, input_size, output_size,
                                        dnn_active_fuc[0])
    ...
    for i in range(len(dnn_layer) - 1):
        with tf.variable_scope("deep_layer%d" % (i + 2), reuse=tf.AUTO_REUSE):
            print("lay%s, input_size: %s, output_size: %s, active_fuc: %s" % (
                i + 2, dnn_layer[i], dnn_layer[i + 1], dnn_active_fuc[i + 1]))
            # 全连接计算
            deep_outputs = udf_full_connect(deep_outputs, dnn_layer[i],
                                            dnn_layer[i + 1],
                                            dnn_active_fuc[i + 1])
    ...
    print("lay_last, input_size: %s, output_size: %s, active_fuc: %s" %
          (dnn_layer[-1], 2, dnn_active_fuc[-1]))
    with tf.variable_scope(
            "deep_layer%d" % (len(dnn_layer) + 1), reuse=tf.AUTO_REUSE):
        deep_outputs = udf_full_connect(deep_outputs, dnn_layer[-1], 2,
                                        dnn_active_fuc[-1])
...
with tf.name_scope("losses"):
    with tf.name_scope("error_loss"):
        ...
        print("y", tf.shape(y))
        error_loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(
                logits=tf.reshape(Y_sum, [-1]),
                labels=tf.reshape(
                    tf.cast(
                        tf.argmax(
                            y, axis=1), tf.float32), [-1])))
...
with tf.Session() as sess:
    ...
    for i in range(training_steps):
        ...
        summary, voliadata_accuracy = sess.run([merged, accuracy],
                                               feed_dict={x: data,
                                                          y: label})
        ...
        if i % 100 == 0 or i <= 100:
            ...
            print("After %d trainging steps ,validation accuarcy is %g%%" %
                  (i, voliadata_accuracy[0] * 100))
        #xs,ys =data.train.next_batch(200)
        #sess.run(train_op,feed_dict={x:xs,y:ys})

    print("Testing Accuracyis %g%%" % (accuracy[0].eval({
        x: X_test,
        y: y_test
    }) * 100))
writer.close()
