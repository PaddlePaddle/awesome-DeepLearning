...
class SubWordTools:
    ...
    def gen_ngrams(self, sequence):
        ...
        for w in sequence:
            subs += [
                self.gen_hash_code(sub)
                for sub in self.gen_subs(self.id2word_dict[w])
            ]
        for n in self.ngrams:
            for i in range(len_seq - n + 1):
                ...
                subs += [
                    self.gen_hash_code(sub) for sub in self.gen_subs(grams)
                ]
        len_subs = len(subs)
...
class FastTextC_pp:
    ...
    def fit(self, data):
        ...
        words_counter = [(word, count)
                         for word, count in Counter(chain(*data)).most_common()
        ...
        self.word2id_dict = {
            word: id
            for id, (word, _) in enumerate(words_counter)
        }
        self.id2word_dict = {
            id: word
            for id, (word, _) in enumerate(words_counter)
        ...
        self.word2id_freq = {
            id: freq
            for id, (_, freq) in enumerate(words_counter)
        }
        # transfer data to ids
        data = [[
            self.word2id_dict[word] for word in line
        ...
        self.subwordtool = SubWordTools(self.id2word_dict, self.threads,
                                        self.sub_ngrams, self.ngrams,
                                        self.sequence_length, self.hash_bins)
    ...
    def subsampling(self, data):
        ...
        skip_prob = {
            id: 1 - (self.skip_ratio / freq * total_num)**0.5
            for (id, freq) in self.word2id_freq.items()
        }
        return [[word for word in line if np.random.rand() > skip_prob[word]]
                for line in data]
    ...
    def forward(self, inputs, words_count, label=None):
        ...
        :return:
        """
        sequence_emb = fluid.embedding(
            inputs,
            size=[self.hash_bins, self.embedding_size],
            is_sparse=True,
            padding_idx=0,
            param_attr='shred_w')
        sequence_emb = fluid.layers.reduce_sum(sequence_emb, dim=1)
        words_count = fluid.layers.reshape(words_count,
                                           [words_count.shape[0], 1])
        sequence_emb = fluid.layers.elementwise_div(sequence_emb, words_count)
        pred = fluid.layers.fc(input=[sequence_emb],
    ...
    def train(self, inputs, label, epochs=5, batch_size=32):
        if not self.deployed:
            input_var = fluid.data(
                name='inputs',
                shape=[None, self.sequence_length],
                dtype='int64')
            word_count_var = fluid.data(
                name='word_count', shape=[None], dtype='float32')
            label_var = fluid.data(
                name='label', shape=[None, self.output_dim], dtype='int64')
            ...
            loss = fluid.layers.cross_entropy(
                pred, label_var, soft_label=False)
        ...
        feed_var_list_loop = [
            self.main_program.global_block().var(var_name)
            for var_name in feed_order
        ]
        self.feeder = fluid.DataFeeder(
            feed_list=feed_var_list_loop, place=self.place)

        self.history = {'loss': [], 'accuracy': []}
        for epoch in range(epochs):
            data = self.subwordtool.build_data(
                inputs, label, batch_size=batch_size)
            ave_loss = 0
            ave_acc = 0
            step = 0
            start = time.time()
            ...
            for data_count_batch, labels_batch in data:
                ...
                loss_, accuracy = self.exe.run(
                    self.main_program,
                    feed=self.feeder.feed(
                        zip(data_batch, num_batch, labels_batch)),
                    fetch_list=[self.loss, self.acc])
                step += 1
                ave_loss = (ave_loss *
                            (float(step) - 1.0) + loss_[0]) / float(step)
                ave_acc = (ave_acc *
                           (float(step) - 1.0) + accuracy[0]) / float(step)
                if step % 10 == 0 and step < total_step:
                    print(
                        'epoch: {} - ETA: {:.2f} - step: {} / {} - acc: {:.4f} - loss: {:.4f}'.
                        format(epoch + 1, (time.time() - start) / step * (
                            total_step - step),
                               str(step).ljust(len(str(total_step))),
                               total_step, accuracy[0], loss_[0]))
                if step == total_step:
                    print(
                        'epoch: {} done - time cost: {:.2f} -ave acc: {:.4f} - ave loss: {:.4f}'.
                        format(epoch + 1,
    ...
    def evaluate(self, test_inputs, test_labels, batch_size=32):
        ...
        for data_count_batch, labels_batch in data:
            ...
            if step % 10 == 0 and step < total_step:
                print(
                    'ETA: {:.2f} - step: {} / {} - acc: {:.4f} - loss: {:.4f}'.
                    format((time.time() - start) / step * (total_step - step),
                           str(step).ljust(len(str(total_step))), total_step,
                           accuracy[0], loss_[0]))
            if step == total_step:
                print(
                    'Done - time cost: {:.2f} -ave acc: {:.4f} - ave loss: {:.4f}'.
                    format(time.time() - start, ave_acc, ave_loss))
    ...
    def predict_prob(self, inputs, batch_size=32):
        ...
        for data_count_batch in data:
            ...
            labels_batch = np.zeros(
                (data_batch.shape[0], self.output_dim)).astype('int64')
...
for i, j in zip(test_labels, pred_labels):
    print('True label is {}, Predicted label is {}'.format(label_set[i],
                                                           label_set[j]))
