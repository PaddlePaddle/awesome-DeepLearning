    ...
    def __init__(self, mode='train'):
        ...
        if mode == 'train':
            ...
            for i in range(len(text)):
                self.data.append([])
                self.data[-1].append(
                    np.array([int(i) for i in text[i].split(",")]))
                self.data[-1][0] = self.data[-1][0][:256].astype(
                    'int64') if len(self.data[-1][
                        0]) >= 256 else np.concatenate([
                            self.data[-1][0], np.array([dict_txt["<unk>"]] * (
                                256 - len(self.data[-1][0])))
                        ]).astype('int64')
...
class myLSTM(nn.Layer):
    def __init__(self):
        ...
        self.lstm = nn.LSTM(
            256, 256, num_layers=2, direction='bidirectional', dropout=0.5)
...
lstm_model.prepare(
    paddle.optimizer.Adam(
        learning_rate=0.001, parameters=lstm_model.parameters()),
...
GRU_model.prepare(
    paddle.optimizer.Adam(
        learning_rate=0.001, parameters=GRU_model.parameters()),
    paddle.nn.CrossEntropyLoss(),
    paddle.metric.Accuracy())
