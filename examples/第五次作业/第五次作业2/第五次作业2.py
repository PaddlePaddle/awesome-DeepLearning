...
def build_data(corpus,
    ...
    while center_word_idx < len(corpus):
        ...
        context_word_range = (max(0, center_word_idx - window_size), min(
            len(corpus) - 1, center_word_idx + window_size))
        context_word_candidates = [
            corpus[idx]
...
def build_batch(dataset, batch_size, epoch_num):
    ...
    for epoch in range(epoch_num):
        ...
        for context_word, target_word, label in dataset:
            ...
            if len(context_word_batch) == batch_size:
                yield epoch, np.array(context_word_batch).astype(
                    "int64"), np.array(target_word_batch).astype(
                        "int64"), np.array(label_batch).astype(
                            "float32"), np.array(eval_word_batch).astype(
                                "int64")
                context_word_batch = []
                target_word_batch = []
                label_batch = []
    ...
    if len(context_word_batch) > 0:
        yield epoch, np.array(context_word_batch).astype("int64"), np.array(
...
class SkipGram(fluid.dygraph.Layer):
    ...
    def forward(self, context_words, target_words, label, eval_words):
        ...
        word_sim = fluid.layers.elementwise_mul(context_words_emb,
                                                target_words_emb)
        ...
        word_sim_on_fly = fluid.layers.matmul(
            eval_words_emb, self.embedding._w, transpose_y=True)
...
with fluid.dygraph.guard(fluid.CUDAPlace(0)):
    ...
    for epoch_num, context_words, target_words, label, eval_words in build_batch(
        ...
        step += 1
        if step % 100 == 0:
            print("epoch num:%d, step %d, loss %.3f" %
                  (epoch_num, step, loss.numpy()[0]))
