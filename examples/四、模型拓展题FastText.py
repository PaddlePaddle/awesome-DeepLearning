...
# # 原理介绍
# 
# 作者提出了一种线性分类器进行分类。其原理也比较容易理解，如图为原文中对于分类器的解释：
# 
# ![](https://ai-studio-static-online.cdn.bcebos.com/4ea4543ad49240939f43461f4977493a8d0083cbc05d46a8b018fce22ed44831)
# 
# 作者将文档样本中所有词的词向量相加求平均，便得到了该文档样本的特征表示，然后用该向量经过一个全连接层，最后输出分类结果。需要注意的是，文章并没有用深度学习框架进行实现，因为这个分类器结构并不是一个很深的神经网络。其隐含层中只有相加求平均后的词向量，然后经过softmax归一，推测分类的结果。为了运行速度，文章还提出了Huffman树处理不同的分类，使softmax归一的速度大幅提高。
# 
# 为了考虑文本中相邻词之间的顺序关系，引入了n-grams作为特征，对词进行补充。2-grams即考虑两个相邻词当做一个新词，3-grams即考虑三个相邻词当做一个新词。这个新词重新学习其词向量。最后文本样本的向量表示为所有词，以及n-grams词的向量之和。由于n-grams的数量远远大于词的数量，所以文章也引入了特征哈希的方法，对n-grams的内存占用进行了优化（在损失后向传播的过程中，嵌入向量集的数量会大大影响运算速度，因为查词的过程也是消耗时间的，词越多，查询时间越长。）想要了解特征哈希方法请参考我之前的[NLP经典之五：是时候认真对待特征哈希了！带你认真细致地了解特征哈希操作](https://aistudio.baidu.com/aistudio/projectdetail/527098)。
# 
# 如果想要更加详细了解这个方法，可以参考这个[网址](https://blog.csdn.net/feilong_csdn/article/details/88655927)
...
# 
# class SubWordTools:
#     def __init__(self, data, id2word_dict, threads=0, sub_ngrams=(3,4,5,6), ngrams=(2,), sequence_length=3000, hash_bins=1e6):
...
#             start = time.time()
#             #batch_data.append(self.gen_ngrams(data[id]))
#             batch_data.append(data[id])
#             cost_time += time.time() - start
#             if labels is not None:
#                 batch_label.append(labels[id])
#             if len(batch_data) == batch_size:
#                 batch_data = p.map(self.gen_ngrams, batch_data)
