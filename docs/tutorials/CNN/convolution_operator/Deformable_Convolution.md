...
但对于视觉识别的传统CNN模块，不可避免的都存在**固定几何结构**的缺陷：卷积单元在固定位置对输入特征图进行采样；汇集层以固定比率降低空间分辨率；一个ROI（感兴趣区域）池层将一个ROI分割成固定的空间单元；缺乏处理几何变换的内部机制等。
...
普通的卷积，以3x3卷积为例对于每个输出$y_(\mathrm{p}_0)$，都要从x上采样9个位置，这9个位置都在中心位置$\mathrm{x}(p0)$向四周扩散，$(-1,-1)$代表$\mathrm{x}(p0)$的左上角，$(1,1)$代表$\mathrm{x}(p0)$的右下角。

$$
\mathrm{R} = 	\{(-1,-1),(-1,0),...,(0,1),(1,1)\}
$$

所以传统卷积的输出就是（其中$\mathrm{p}_n$就是网格中的n个点，$\mathrm{w}(\mathrm{p}_n)$表示对应点位置的卷积权重系数）：
$$
...
但DCN v1听起来不错，但其实也有问题：我们的可变形卷积有可能引入了无用的上下文（区域）来干扰我们的特征提取，这显然会降低算法的表现。通过图4的对比实验结果（多边形区域框）我们也可以看到DCN v2更能集中在物体的完整有效的区域
