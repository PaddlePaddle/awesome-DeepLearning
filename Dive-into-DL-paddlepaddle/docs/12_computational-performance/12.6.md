...
```
    ...
    net = paddle.DataParallel(net) # 分布式model，用于支持分布式训练
...
当然，飞桨已经通过Spawn分布式训练套件帮我们实现了，我们只需要导入和初始化Spawn分布式训练，配置网络的分布式，使用`dist.spawn(train)`调用训练函数，在命令行下使用`python xx.py`直接执行即可。
...
在notebook环境下使用`train(batch_size=256, lr=0.05)`。如果要多卡并行，则需要在文件中使用`dist.spawn(train, args=(256, 0.1)) ` 并到命令行下执行。
...
飞桨的多卡训练，需要在命令行下使用python  xx.py格式方式执行。同时飞桨的多卡并行非常智能，会自动识别GPU卡的数量，默认使用全部GPU卡进行训练！

下面我们使用`%%writefile multigpu.py`魔法命令将代码写入文件，然后在notebook中使用`!python  multigpu.py`执行即可。
...
# 飞桨多GPU程序，不管单GPU环境还是多GPU环境，都可以在不修改代码，不修改参数，不修改执行命令的情况下正常执行。命令使用python  multigpu.py格式方式执行。
import paddle
from paddle import nn
import d2l.paddle as d2l
...
if __name__ == "__main__" :
    # 若为多GPU，学习率需要乘以相应倍数以达到较好效果，比如2GPU lr=0.1 4GPU lr=0.2
    dist.spawn(train, args=(256, 0.1)) 
...
* 飞桨的多卡并行分布式训练非常简单方便，不用调整原来的网络结构，只要加上四处代码即可。
...
使用dist.spawn(train, args=(256, 0.1)) 这样的函数调用
```

* 所有的数据并行处理，以及GPU卡的管理，统一由并行模块Spawn处理，省心省力。
