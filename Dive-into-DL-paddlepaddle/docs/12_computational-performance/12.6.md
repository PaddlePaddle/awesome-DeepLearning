...
```
    ...
    paddle.set_device("gpu") # 多卡需要设置这句
    
    # 初始化Fleet环境
    fleet.init(is_collective=True)

    resnet = resnet18(10) # resnet10分类
...
当然，飞桨已经通过fleet分布式训练套件帮我们实现了，我们只需要初始化fleet分布式训练，配置网络和优化器的分布式训练就行了。



```python
def train(batch_size=256, lr=0.05):
    momentum_rate = 0.9 # 冲量
    l2_decay = 1e-4 # 权重衰减
    devices = d2l.try_all_gpus() # 获得GPU字符串，只是为了最后显示GPU列表，不影响实际训练。
    ...
    for epoch in range(num_epochs):
        ...
        for batch_id, data in enumerate(train_iter):
            ...
            if batch_id % 50 == 0: # 每50个batch显示训练信息，去掉该代码不影响实际训练
...
在notebook环境下使用`train(batch_size=256, lr=0.05)`命令只在单GPU环境下有效，如果是多卡环境，会报错`CUDA_VISIBLE_DEVICES shoule be set only 1 card if you use 'python' to launch fleet program.` 
...
飞桨的多卡训练，需要在命令行下使用python -m paddle.distributed.launch xx.py格式方式执行。同时飞桨的多卡并行非常智能，会自动识别GPU卡的数量，默认使用全部GPU卡进行训练！

下面我们使用`%%writefile multigpu.py`魔法命令将代码写入文件，然后在notebook中使用`# !python -m paddle.distributed.launch multigpu.py`执行即可。

刚才我们知道了，飞桨会自动识别GPU卡的数量，那么神奇的地方就是，在不修改代码和参数，也不修改执行命令的情况下，我们前面写的代码在单卡环境和多卡环境下都能正常运行！
...
# 飞桨多GPU程序，不管单GPU环境还是多GPU环境，都可以在不修改代码，不修改参数，不修改执行命令的情况下正常执行。命令使用python -m paddle.distributed.launch xx.py格式方式执行。
import paddle
from paddle import nn
import d2l.paddle as d2l
...
# 训练
def train(batch_size=256, lr=0.1):
    momentum_rate = 0.9 # 冲量
    l2_decay = 1e-4 # 权重衰减
    devices = d2l.try_all_gpus() # 获得GPU字符串，只是为了最后显示GPU列表，不影响实际训练。
...
if __name__ == "__main__" :
    train(batch_size=256, lr=0.05) # 若为多GPU，学习率需要乘以相应倍数以达到较好效果，比如2GPU lr=0.1 
...
* 飞桨的分布式训练模块Fleet非常简单方便，不用调整原来的网络结构，只要加上三处代码即可。
...
* 所有的数据并行处理，以及GPU卡的管理，统一由Fleet处理，省心省力。
* 飞桨的分布式训练代码不用任何修改，就可以适应单卡、双卡以及多卡的各种环境。
