...
![](https://ai-studio-static-online.cdn.bcebos.com/7b52308fb57d442f854d4b0320a105eb31f2fc958ee94f6680f84c49695ab476)
...
```python
train_imgs = paddle.vision.datasets.DatasetFolder(os.path.join(data_dir, 'train'))
test_imgs = paddle.vision.datasets.DatasetFolder(os.path.join(data_dir, 'test'))
```
...
train_augs = paddle.vision.transforms.Compose([
    paddle.vision.transforms.RandomResizedCrop(224),
...
```python
pretrained_net = paddle.vision.models.resnet18(pretrained=True)
```
...
```python
finetune_net = paddle.vision.models.resnet18(pretrained=True)
...
```python
scratch_net = paddle.vision.models.resnet18()
