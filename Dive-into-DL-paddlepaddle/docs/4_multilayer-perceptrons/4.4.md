...
```
    ...
    <ipython-input-7-3c8eaf3f8147> in <module>
          1 # 从多项式特征中选择前4个维度，即 1, x, x^2/2!, x^3/3!
    ----> 2 train(poly_features[:n_train, :4], poly_features[n_train:, :4],
          3       labels[:n_train], labels[n_train:])


    <ipython-input-6-b1bf7ee29438> in train(train_features, test_features, train_labels, test_labels, num_epochs)
         16                             legend=['train', 'test'])
         17     for epoch in range(num_epochs):
    ---> 18         pd2l.train_epoch_ch3(net, train_iter, loss, trainer)
         19         if epoch == 0 or (epoch + 1) % 20 == 0:
         20             animator.add(epoch + 1, (evaluate_loss(net, train_iter, loss),
...
```
    ...
    <ipython-input-10-e20d3de8ba76> in <module>
          1 # 从多项式特征中选取所有维度
    ----> 2 train(poly_features[:n_train, :], poly_features[n_train:, :],
          3       labels[:n_train], labels[n_train:], num_epochs=1500)


    <ipython-input-7-0f6e85eca3b1> in train(train_features, test_features, train_labels, test_labels, num_epochs)
          4     input_shape = train_features.shape[-1]
          5     # 不设置偏置，因为我们已经在多项式特征中实现了它
    ----> 6     net = nn.Sequential(nn.Linear(input_shape, 1, bias=False))
          7     batch_size = min(10, train_labels.shape[0])
          8     train_iter = d2l.load_array((train_features, train_labels.reshape(-1,1)),
